...
export class UsersService {
  ...
  async getUsers(params: GetUsersDto): Promise<ResponsePaginate<UserDto>> {
    ...
    if (params.startDate && params.endDate) {
      query = query.andWhere('users.createdAt BETWEEN :startDate AND :endDate', {
        startDate: params.startDate,
        endDate: params.endDate,
      });
    }

    query = query.orderBy(`users.${params.orderBy}`, params.order).skip(params.skip).take(params.take);
